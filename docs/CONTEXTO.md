...
## 6f. Ronda 8 (28 de agosto)

1. **O CTA do WhatsApp saiu da chapa escura**: era o primeiro bloco do `<footer class="field-deep">` e lia-se como o principio do rodape. Agora e uma `<section class="field-shell rodape__cta">` propria, na cor do que vem por cima, e o escuro comeca depois dela. Vale em todas as paginas, porque o `SiteFooter` e um so.
2. **O rodape encolheu**: a morada, o email, o telefone e o horario sairam e no lugar deles ha um botao "Contactos". Ficam a marca, as duas redes, o botao e a linha legal. O `padding-block` desceu de `--s-lg` para `--s-md`.
3. **As fotos dos paineis dos projetos apareciam TRIPLICADAS em telemovel.** A causa era o `background-image` com `background-size: auto 120%` herdado do interactive-selector: num painel fechado, largo e baixo, a largura calculada a partir da altura fica menor do que o painel e o browser repete a imagem. Passou a `<img class="paineis__foto">` com `object-fit: cover`, e a ampliacao do painel fechado passou a `transform: scale(1.12)` na propria imagem. Nunca estica: no maximo amplia e corta.
4. **O logotipo animado da pagina de contactos** ficou maior (`tamanho="enorme"`) e ganhou `orbita`: a oval deixa de se desenhar e apagar, fica desenhada e roda a volta do logotipo, 16s por volta, linear. ⚠️ Duas contas que morderam: a largura TEM de incluir a margem (a 78vw mais 3rem de cada lado o conjunto passava dos 375px, alargava o documento e cortava o texto da pagina toda pela direita), e a oval roda encolhida a 78% porque nao esta centrada no logotipo e a 100% o circulo que ela varre entrava pelo paragrafo de cima.

## 6g. Ronda 9 (28 de agosto)

1. **A oval dos contactos estava torta e a rodar como um todo.** O `orbita` fazia a oval desenhada a mao (que e um traco aberto, de proposito) rodar sobre si: inclinava-se e lia-se torta. Agora, so nessa pagina, o traco a mao da lugar a uma `<ellipse>` a serio, direita e fechada, **parada**; o que anda e o TRACEJADO dela. `pathLength={1000}` faz o perimetro contar sempre 1000, e com tracos de 20 e intervalos de 20 cabem 25 pares certos: deslocar o tracejado exactamente 40 fecha a volta sem salto.
2. **O formulario de contacto passou a TRES PASSOS** com barra de progresso: "O pedido" (servico e distrito), "Contacto" (nome, telefone, email opcional) e "Detalhes" (nota opcional mais a pre-visualizacao do que vai seguir). A validacao e por passo e corre ao carregar em "Seguinte", com o foco a saltar para o primeiro campo que falta; o botao nunca fica desactivado. No fim continua a abrir o WhatsApp ja escrito, e ha um ecra de sucesso com o texto, um link de recurso caso a janela seja bloqueada, e um botao para copiar. **O site do Tomas Marques nao tem formulario de passos** (o que ele tem sao tres cartoes editoriais numerados); este foi construido de raiz na linguagem da casa.
...
## 6i. Ronda 11 (30 de agosto)

1. **O botao "Pedir proposta" da folha do menu em telemovel nao fazia nada.** Apontava para `${ROTAS.home}${ANCORAS.contacto}`, ou seja `/#falar`, e nao ha nenhum elemento com esse id em lado nenhum: o formulario vive em `/contacto/` e a homepage nao tem seccao de formulario. Passou a `ROTAS.contacto`, como o CTA de desktop, e ganhou `onClick={() => setAberto(false)}` para fechar a folha, que era o unico link ali sem isso. O `ANCORAS` deixou de ser importado no `site-nav.tsx`.
2. **O gesto da marca saiu de todo o lado e no lugar dele entrou a marca a NASCER.** Nada de sublinhados nem de contorno a correr: e so o icone deles (`public/marca/icone.png`, os dois paineis e o sol, sem o nome), com os paineis ja postos e o sol a nascer atras, primeiro o arco e depois os raios um a um. Video feito no Higgsfield com `seedance_2_5` em `mode: "omni_reference"`, `start_image` branco e `end_image` com o icone, 1:1, 720p, 5 s, 32,5 creditos. **Do video so se usa a partir do segundo 2**, por decisao do Tomas: o nascimento dos paineis fica de fora e o que se ve e o sol a nascer.
   - `scripts/marca-nasce.py` faz o resto e tem as quatro decisoes anotadas no cabecalho. As duas que custaram tempo: **o chao do alfa e 0,07 e nao zero** (o "branco" do H.264 anda nos 250 a 253 e deixava um rectangulo cinzento por cima do degrade), e **o alfa guarda-se em 8 niveis** (com alfa completo o ficheiro dava 741 KB, com 8 niveis da 346 KB, sem perdas na cor; e a cauda que desvanece para o icone tem de ser requantizada, senao reintroduz 256 niveis em 6 fotogramas e volta aos 420 KB).
   - ⚠️ **Nao tentar `mix-blend-mode: multiply` para fugir ao alfa.** Ja se tentou: `.campo-luz__conteudo` tem `z-index: 1` e a lavagem do `campo-luz__luz` fica FORA desse contexto de empilhamento, por isso o multiply nao tem nada por baixo com que misturar e fica uma chapa branca a volta do logotipo.
   - ⚠️ **O componente e um `<picture>` e nao duas `<img>`.** A primeira versao punha a animacao e o icone parado no DOM e escondia um por CSS para o `prefers-reduced-motion`; bastou o browser servir CSS em cache para aparecerem os dois sobrepostos. Com `<picture>` + `<source media="(prefers-reduced-motion: reduce)">` so existe um elemento e a escolha e do browser.
   - O recorte dos fotogramas e calculado para dar **exactamente o enquadramento do `icone.png`**, e o ultimo fotograma E esse ficheiro: o estado de repouso e o logotipo do cliente e nao a versao do modelo.
   - Fica em `ui/marca-nasce.tsx` (tamanhos `grande` 9rem, `media` 8rem, `pequena` 7rem), no Quem somos da home e na Qualidade do Sobre. O `ui/marca-animada.tsx` e o bloco "O GESTO DA MARCA" do `globals.css` foram apagados.
...
## 6l. Ronda 11d (31 de agosto, madrugada)

1. **A fita dos projetos tem desenho proprio** (`diagonal` no `fita-solta.tsx`): entra pelo canto inferior esquerdo da caixa, faz duas ondas no meio e sai na parede direita quase no topo. Nas duas medidas. A dos servicos continua a ser o caminho do Sobre (`desce`).
2. ⚠️ **A linha partia em pedacos**: `pathLength={1}` normalizado junto com `vector-effect: non-scaling-stroke` faz o Chrome escalar mal o tracejado. A cura tem DUAS partes: (a) dasharray/dashoffset com o COMPRIMENTO REAL do caminho via `--traco` (2936 na fita do Sobre, 5193 na diagonal), medido por amostragem das cubicas; (b) o desenho novo em coordenadas GRANDES (viewBox 0 0 3000 3000 e nao 0 0 100 100), porque um viewBox ampliado para o ecra volta a partir o traco mesmo com comprimentos reais; reduzido porta-se bem.
3. **O avanco abrandou sem mexer no recuo**: keyframes proprias `fita-solta-desenha` (16s: 58% a desenhar ~9,3s, pausa, recuo ~5,6s como estava). O Tomas tinha razao: com as janelas do Sobre o desenhar lia-se apressado.
4. **O sol do logotipo a nascer, em CSS puro** (`ui/sol-nasce.tsx`): o arco desenha-se em circulo (300 graus, aberto em baixo como no icone) e depois nascem os sete raios um a um; no fim o conjunto desvanece e recomeca (ciclo 12s). Na tinta da fita, opacity 0.55. Nos dois indices, em sitios diferentes (`--projetos` cima/direita, `--servicos` mais abaixo). ⚠️ Cada raio tem as SUAS keyframes (sol-raio-1..7) geradas estaticamente, e comprimentos reais no dasharray pela mesma razao do ponto 2.
5. **Intros das paginas de servico com realce**, como as fichas de projeto: marcas `**` no `site.ts` (2 a 3 por servico, 14 no total) e `TextoRealcado` no `servicos/[slug]/page.tsx` no lugar do paragrafo simples.
6. **A fita de servicos da homepage ganhou setas** esquerda/direita (`fita__seta` no `fita-servicos.tsx`): um cartao por clique via `scrollBy` suave, calam o automatico como qualquer gesto. ⚠️ Vivem no `.fita-conjunto` FORA da `.fita`, porque a mascara que esbate as pontas esbatia os botoes junto.

## 6m. Ronda 11e (31 de agosto, madrugada)

**A fita verde saiu dos cabecalhos dos indices e o sol substitui-a**, por decisao do Tomas: circulo COMPLETO (sem a abertura do icone), centrado na zona livre a direita do titulo, nas duas paginas. Feito no Higgsfield (`seedance_2_5`, omni_reference, branco -> sol desenhado localmente com PIL como end_image, 4s, 1:1, 720p, 26 creditos): a linha desenha o circulo, o anel ganha corpo e os oito raios saltam um a um. `scripts/sol-nasce.py` converte para `public/marca/sol-nasce.webp` (153 KB, alfa por canal com chao 0,07, 8 niveis, ping-pong) + `sol-parado.png` para prefers-reduced-motion. Componente `ui/sol-nasce.tsx` (o CSS puro de antes foi substituido), `<picture>` como o marca-nasce. Apagados `ui/fita-solta.tsx` e os blocos de CSS da fita solta e do sol CSS. Master em `~/Desktop/bm-sol-nasce.mp4`.

**Pendentes do video do heroi**: prompts do drone (carrinha com o logo -> subida para os trabalhadores no telhado) entregues ao Tomas na conversa de 31/8; fotos primeiro com `nano_banana_pro` + `logo-cor.png` como image_references, video depois no Cinema Studio com start/end image. A espera do OK dele para gerar.

## 6n. Ronda 11f (31 de agosto): o video novo do heroi

O take e o planeado com o Tomas: drone baixo a acompanhar a carrinha Blue Magnitude, sobe a fachada da vivenda de luxo, revela os tres instaladores no telhado e afasta-se para o plano aereo com a carrinha a entrar na garagem. Feito por ele no Higgsfield (Cinema Studio, fotos start/end geradas com nano_banana_pro + logo-cor.png como referencia; master HEVC 10 bits em ~/Downloads/hf_20260831_002546_*.mp4, 1920x1080, 5s).

Processo, com duas correccoes do Tomas ja aplicadas:

⚠️ **O LACO COMECA NO PRIMEIRO FOTOGRAMA E A CABECA ENTRA INVERTIDA.** A primeira tentativa cosia o laco a arrancar o corpo em 0,8s (corpo = 0,8..D, fundido com a cabeca 0..0,8), e o video parecia comecar mais tarde do que o master. Agora o corpo e 0..D-0,8 e o que funde no fim e a cabeca **ao contrario** (`trim=0:0.8,reverse`): assim o ultimo fotograma do laco e exactamente o primeiro (diferenca medida 1,35) e o arranque e o do master (1,18). Sem a inversao o fim caia no fotograma 0,8 e o laco saltava (diferenca 38).

⚠️ **O RECORTE VERTICAL LEVA MENOS ZOOM E BANDAS DESFOCADAS.** Era `crop=607:1080:880:0` (32% da largura do master) e lia-se demasiado perto. Como o `<video>` e `object-fit: cover` num ecra 9:16, mostrar mais campo obriga a nao encher a altura toda: agora o recorte e `crop=800:1080:783:0` escalado a 720x972 e assente sobre uma copia do proprio fotograma ampliada, desfocada (`gblur=sigma=40`) e escurecida, a preencher os 720x1280. As bandas ficam no ceu em cima e por baixo da lavagem do titulo em baixo.

H.264 yuv420p +faststart sem audio: `hero-lg.mp4` 1920x1080 CRF21 (4,4 MB, 4,24s) e `hero-vert.mp4` 720x1280 CRF23 (1,3 MB). Posters webp por cwebp (o ffmpeg do brew nao tem libwebp). Nada mudou no codigo: os nomes dos ficheiros sao os mesmos e o `video-backdrop` fica como esta.

## 6o. Ronda 11g (31 de agosto): tres correccoes do iPhone

1. **O heroi lia-se a andar para tras nos primeiros instantes.** A culpa era da cabeca INVERTIDA no fundido do laco (`trim=0:0.8,reverse`), que fechava o laco ao pixel mas punha 0,8s de movimento ao contrario antes de reiniciar. Agora a cabeca entra para a frente: o laco cruza 0,8s de movimento REPETIDO, que se le como continuidade e nunca como marcha atras. O arranque continua no primeiro fotograma do master (diferenca medida 1,18).
2. **O sol encostava as letras em telemovel.** A 390px o titulo ocupa quase a largura toda; abaixo de 48rem o sol passou a `right: 0.75rem` e `width: 7.75rem`, a meio caminho entre onde estava (2,9rem, colado as letras) e a margem negativa que ficou demasiado encostada ao bordo. Em desktop nao mudou.
3. ⚠️ **O RODAPE E O FIM DO DOCUMENTO.** No iPhone dava para arrastar para baixo do rodape e aparecia meio ecra de fundo vazio. Medido no Chrome a 390px, `scrollHeight` era EXACTAMENTE o fim do `<footer>`, ou seja nao ha altura a mais no documento: o excesso e o elastico do iOS. A cura sao duas linhas em `html, body`: `overscroll-behavior-y: none` (tira o elastico) e `overflow-x: clip` (nada largo demais cria barra nem altura). `clip` e nao `hidden`, que o `hidden` cria contexto de scroll e parte o `position: sticky`.
   **Isto passou a regra da casa**: esta na memoria `feedback_rodape_e_o_fim` e no kit da VPS em `skills/realtor-landing/references/mobile-first.md`, seccao 8, com o teste e a nota de que ele passa no Chrome e o defeito continua no iPhone.
